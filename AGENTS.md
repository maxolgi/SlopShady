...
1. Builds the three WebSRT WASM crates into `static/wasm/` if missing or stale (via `scripts/build-wasm.sh`; gated on an mtime check so untouched builds skip the ~8s wasm-pack overhead).
...
- `static/wasm/` is regenerated by `build.rs` via `scripts/build-wasm.sh` (gitignored).
