...
reproduces the artifacts from that source. `static/wasm/` is gitignored and
regenerated by `build.rs` on demand; to build manually:

```bash
git submodule update --init --recursive   # populate vendor/WebSRT/
cargo build                                # build.rs invokes scripts/build-wasm.sh
```
