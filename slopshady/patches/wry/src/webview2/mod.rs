...
impl InnerWebView {
  ...
  fn init_webview(
    ...
    // Webview handlers
    unsafe { Self::attach_handlers(hwnd, parent, &webview, &mut attributes, &mut token, env)? };
    ...
    if attributes.ignore_tls_errors {
      if let Ok(webview15) = webview.cast::<ICoreWebView2_15>() {
        let _ = unsafe {
          webview15.add_ServerCertificateErrorDetected(
  ...
  unsafe fn attach_handlers(
    ...
    let parent_for_close = parent;
    webview.add_WindowCloseRequested(
      &WindowCloseRequestedEventHandler::create(Box::new(move |_, _| {
        let _ = DestroyWindow(hwnd);
        let _ = PostMessageW(Some(parent_for_close), WM_CLOSE, WPARAM(0), LPARAM(0));
        Ok(())
      })),
      token,
    )?;
