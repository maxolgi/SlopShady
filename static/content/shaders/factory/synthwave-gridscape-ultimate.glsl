// Name: Synthwave Gridscape - Ultimate Neon Edition
// Synthwave Gridscape - Ultimate Neon Edition
// Maximum retro aesthetic with perfect color balance and enhanced effects

#define GRID_SCALE 1.3
#define LINE_WIDTH 0.028
#define SUN_COLOR vec3(1.0, 0.22, 0.08)
#define GRID_COLOR vec3(0.0, 0.92, 1.0)
#define FOG_COLOR vec3(0.15, 0.08, 0.4)
...
void main() {
    ...
    // Background: Vibrant retro gradient with smooth transitions
    float time_shift = sin(iTime * 0.25) * 0.12;
    vec3 sky_top = mix(
        vec3(0.35, 0.18, 0.78 + time_shift), 
        vec3(0.55, 0.25, 0.92 + time_shift), 
        (uv.y + 1.0) * 0.5
    );
    vec3 sky_bottom = mix(
        vec3(0.65, 0.32, 0.72), 
        vec3(0.78, 0.42, 0.68 + time_shift), 
        (uv.y + 1.0) * 0.5
    );
    ...
    if (t > 0.0) {
        ...
        vec3 grid_rgb = mix(vec3(0.25), GRID_COLOR, line * 0.98);
